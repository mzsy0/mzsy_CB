...
使用案例
'timing': {
            'name': '定风波择时_跌停比例',  # 择时策略名称
            'limit': 0,
            'factor_list': [('次日涨跌停状态', False, '跌停', 1, '0945')],
            'params': 0.002
        }       
'''
...
def signal(strategy: StrategyConfig, df_after_limit: pd.DataFrame):
    ...
    # ======================== 计算下跌比例 ===========================
    decl_ratio = pd.DataFrame(df_after_limit.groupby('交易日期')[col_name].mean())
    decl_ratio.rename(columns={col_name: '跌停比例'}, inplace=True)

    # ======================== 返回择时信号 ===========================
    signals = decl_ratio['跌停比例'].le(ratio).astype(int)  # 返回跌停比例小于ratio的交易日，即为选股信号
